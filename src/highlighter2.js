 ...
 * - selection end index
 */
export default class HighLighter2 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      text: props.text,
  ...
  render() {
    if (!this.state.selection) {
      return <div onMouseUp={this.onMouseUpHandler}>{this.state.text}</div>;
    } else {
      return (
        <div onMouseUp={this.onMouseUpHandler}>
          <div data-order="first">{this.state.first}</div>
          <div
            data-order="middle"
          ...
          <div data-order="last">{this.state.last}</div>
        </div>
      );
    }
  }
...
HighLighter2.propTypes = propTypes;
